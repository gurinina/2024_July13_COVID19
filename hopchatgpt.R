...
download_and_format_png <- function(cid, output_dir, width = NULL, height = NULL, background_color = "white") {
  ...
  if (status_code(response) == 200) {
    ...
    # Save intermediate image after resizing
    intermediate_path_resize <- file.path(output_dir, paste0(cid, "_resized.png"))
    image_write(img, path = intermediate_path_resize, format = "png")
    ...
    plot(img)
    # Save intermediate image after setting background
    intermediate_path_background <- file.path(output_dir, paste0(cid, "_background.png"))
    image_write(img, path = intermediate_path_background, format = "png")
...
cids <- readRDS("24Jan8/hopcids.RDS")
output_dir <- "www/hopimg"
width <- 200
height <- 200
