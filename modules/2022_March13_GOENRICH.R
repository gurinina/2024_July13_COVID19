...
runGOENRICH = function (fdrThresh = 0.2, curr_exp = "test", score, minSetSize =5, bp_path =  "2021_Decenber30_GO_BP.RDS",go_path = "2021_Decenber30_GOID_GOBP_SGD.txt"){
  ...
  maxSetSize = 300, uniSize = NA)
{
  ...
  enrichInfo <- t(enrichInfo)
    enrichCol <- data.frame(term = names(geneSets),
               ...
               FDR = p.adjust(as.numeric(enrichInfo[,4]),
               method = "BH"),
               overlapGenes = enrichInfo[, 1],
               maxOverlapGeneScore = as.numeric(enrichInfo[, 5]),
               stringsAsFactors = F)
